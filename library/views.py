...
class MemberViewSet(viewsets.ModelViewSet):
    ...
    def top_active(self, request):
        members = Member.objects.annotate(active_loans=Count('loans', filter=Q(loans__is_returned=False))).order_by('-active_loans')[:5]
        serializer = TopActiveMembersSerializer(members, many=True)
    ...
    def extend_due_date(self, request, pk=None):
        ...
        if loan.due_date >= timezone.now().date():
            if additional_days > 0:
               ...
               return Response({'error': 'Additional days invalid.'}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({'error': 'Loan due date is expired.'}, status=status.HTTP_400_BAD_REQUEST)
